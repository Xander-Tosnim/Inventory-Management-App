...
export default function SignInForm() {
  ...
  return (
    <form action={formAction}
      ...
      <div>
        <label>Don't have an account? <a href="/sign-up" className='cursor-pointer underline hover:no-underline'>Create Account</a></label>
      </div>
      ...
      <button type="submit" disabled={isPending}
        className="flex w-sm justify-center mt-1 rounded-md cursor-pointer transition-all duration-500 rounded-t-2xl bg-indigo-500 px-3 py-1.5 text-sm/6 font-semibold text-white hover:bg-indigo-400">
        Sign in
      </button>
      <Link href="/"
        className="flex w-sm justify-center rounded-md rounded-b-2xl transition-all duration-500 border-2 px-3 py-1.5 text-sm/6 font-semibold text-white hover:bg-white hover:text-black hover:border-indigo-700">
        Home Page
      </Link>
    </form>
  );
